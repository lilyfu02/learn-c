...
CmdNode *newCmdNode(char cmd[10], int val) {
   ...
   */
    CmdNode * node = NULL;
    node = (CmdNode *) malloc(sizeof(CmdNode)); 

    const char * VAILD_CMDS[] = {
      "penup", "pendown", "colour", "forward", "backward", "right", "left","loop"
    };
...
/*---------------------------------------------------------------------------*/
void printCommandListLoop(CmdNode *head,int *count, int level);
void printCommandList(CmdNode *head) {
  int count = 0;
  printCommandListLoop(head,&count, 0);
}
...
void printCommandListLoop(CmdNode *head,int *count, int level) {
  ...
  while(p != NULL){
    if(strcmp(p->cmd,"penup") == 0 || strcmp(p->cmd,"pendown") == 0 || strcmp(p->cmd,"right") == 0 || strcmp(p->cmd,"left") == 0 ){
      ...
      printf("%3d: %s%s %d\n",*count,blank,p->cmd,p->val);
      if(strcmp(p->cmd,"loop") == 0){
        (*count)++;
        printCommandListLoop(p->loop_head,count, level+1);
      }
    }
    (*count)++;
    p = p -> next;
...
/*---------------------------------------------------------------------------*/
void queryByCommandLoop(CmdNode *head, char cmd[10], int *count);
void queryByCommand(CmdNode *head, char cmd[10]) {
  int count = 0;
  queryByCommandLoop(head, cmd, &count);
}
void queryByCommandLoop(CmdNode *head, char cmd[10], int *count) {
  /**
  ...
  while(p != NULL){
    if(strcmp(cmd,p->cmd) == 0){
      ...
      (*count)++;
      queryByCommandLoop(p->loop_head, cmd, count);
    }
    (*count)++;
    p = p->next;
  }
...
void insertCommandLoop(CmdNode *loop_node, CmdNode *new_cmd){
  loop_node->loop_head = insertCommand( loop_node->loop_head, new_cmd);
}
/*---------------------------------------------------------------------------*/
...
void runLoop(Image *im, CmdNode *head, int *endX, int *endY, int *direct, int *pendown, int *col) {
  /**
    ...
    if(strcmp(p->cmd,"penup") == 0){
      ...
      *col = p->val;
      ...
      for(int i = 0; i< (p->val);i++){
        runLoop(im, p->loop_head, endX, endY, direct, pendown, col);
      }
    }
...
void run(Image *im, CmdNode *head, int *endX, int *endY) {
  ...
  *endY = 0;
  ...
  int pendown = 1;
  int col  = 0;
  runLoop( im, head, endX, endY, &direct,&pendown,&col);
}
